...
//! features. [`EvocMatrix`] is the single conversion point that lets both entry
//! points take a faer matrix, an ndarray 2-D array or a pre-flattened row-major
//! buffer without any of the pipeline caring which.
//!
//! The conversion borrows wherever the source can be viewed as a matrix, which
//! is every layout except a non-contiguous ndarray. That matters: the kNN stage
...
/// Borrowed-or-owned matrix produced by [`EvocMatrix::to_mat_input`].
///
/// Owning the awkward case keeps every other one free. Bind the result, then
/// call [`MatInput::as_mat_ref`] for the `MatRef` the pipeline wants.
...
/// Anything that can be handed to [`crate::evoc`] as a samples-by-features
/// matrix.
///
